...
const clearDropClasses = (root: HTMLElement) => {
    root
        .querySelectorAll('.definition-item--drop-before, .definition-item--drop-after, .definition-item--drop-inside')
        .forEach((el) => el.classList.remove('definition-item--drop-before', 'definition-item--drop-after', 'definition-item--drop-inside'));
};
...
const setupRenameDelete = (
    ...
    root.addEventListener('click', (event) => {
        const button = (event.target as HTMLElement).closest<HTMLButtonElement>('.definition-action');
        if (!button) return;
        ...
        if (action === 'rename') {
            ...
            form.addEventListener('submit', (ev) => {
                ...
                sendRequest(htmx, `${base}/editor/definitions-rename`, { id, title: value }, target);
                closeModal();
            });
        ...
        if (action === 'delete') {
            ...
            container.querySelector('[data-modal-close]')?.addEventListener('click', (ev) => {
                ev.preventDefault();
                closeModal();
            });
            container.querySelector('[data-modal-confirm]')?.addEventListener('click', () => {
                sendRequest(htmx, `${base}/editor/definitions-delete`, { id }, target);
                closeModal();
            });
            return;
        }
...
const setupDragAndDrop = (root: HTMLElement, base: string, htmx: HTMX, target: HTMLElement) => {
    let dragContext: DragContext | null = null;
    let dropContext: DropContext = { item: null, position: null };

    const updateDropHighlight = (item: HTMLElement | null, position: DropPosition | null) => {
    ...
    root.addEventListener('dragstart', (event) => {
        const targetNode = (event.target as HTMLElement).closest<HTMLElement>(listSelectors.node);
    ...
    root.addEventListener('dragend', () => {
        if (dragContext) {
            ...
            dragContext.item.querySelector(listSelectors.node)?.classList.remove('definition-node--dragging');
        }
        dragContext = null;
    ...
    root.addEventListener('dragover', (event) => {
        ...
        const item = (event.target as HTMLElement).closest<HTMLElement>(listSelectors.item);
    ...
    root.addEventListener('drop', (event) => {
        ...
        if (dropPosition === 'inside') {
            parentId = targetId;
            const childList = targetItem.querySelector<HTMLElement>(':scope > ul');
    ...
    root.addEventListener('dragover', (event) => {
        ...
        if (!root.contains(target) || target.closest(listSelectors.item)) return;
        event.preventDefault();
        dropContext = { item: null, position: null };
    });
    ...
    root.addEventListener('drop', (event) => {
        ...
        event.preventDefault();
        const rootItems = Array.from(root.querySelectorAll<HTMLElement>(`${listSelectors.item}:not(${listSelectors.item} ${listSelectors.item})`));
...
export default function init(el: HTMLElement) {
    ...
    const modalRoot = document.getElementById('definitions-modal') as HTMLElement | null;
    const createTemplate = document.getElementById('definition-create-template') as HTMLTemplateElement | null;
    const parentSelectCache = document.getElementById('definition-parent-select') as HTMLElement | null;
    const openCreateButton = document.getElementById('definition-open-create') as HTMLButtonElement | null;
    ...
    const openModal = (title: string, body: HTMLElement) => {
        ...
        `;
        const bodyContainer = modalRoot.querySelector('.definitions-modal__body') as HTMLElement;
    ...
    if (openCreateButton && modalRoot && createTemplate && parentSelectCache) {
        openCreateButton.addEventListener('click', () => {
            ...
            if (slot) {
                ...
                const hiddenInput = clone.querySelector<HTMLInputElement>('input[name="parent_id"]');
                if (hiddenInput) {
                    hiddenInput.id = 'definition-parent-value-modal';
                    hiddenInput.value = '';
                }
                const button = clone.querySelector<HTMLButtonElement>('#definition-parent-button');
                if (button) {
                    button.id = 'definition-parent-button-modal';
                    button.setAttribute('aria-labelledby', 'definition-parent-label definition-parent-button-modal');
                }
            ...
            form.addEventListener('htmx:afterRequest', (event) => {
                const detail = (event as CustomEvent).detail as { xhr?: XMLHttpRequest } | undefined;
