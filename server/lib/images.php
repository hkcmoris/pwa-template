...
function img__set_error(string $msg): void
{
    $GLOBALS['__img_last_error'] = $msg;
}
function img_last_error(): string
{
    return (string)($GLOBALS['__img_last_error'] ?? '');
}
        ...
        if (is_dir($full)) {
            ...
            if (in_array($ext, ['webp','jpg','jpeg','png','gif','avif','svg'], true)) {
              // Skip generated thumbnails (e.g., *.thumb.webp)
                if ($ext === 'webp' && preg_match('#\.thumb\.webp$#i', $entry)) {
                    continue;
                }
                $thumbUrl = $url;
                if ($ext === 'webp') {
...
function img_convert_to_webp(string $tmpFile, string $destPath): bool
{
  ...
  // If the image is likely too large for GD, try external tool first
    if (!img__gd_can_decode($info)) {
        ...
        if (img__try_cwebp($tmpFile, $destPath, 85)) {
            return true;
        }
