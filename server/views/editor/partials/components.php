...
<style>
  ...
  .component-action{border:1px solid var(--primary);background:transparent;color:var(--primary);border-radius:.25rem;padding:.25rem .5rem;font-size:.75rem;cursor:pointer}

  .component-action:hover,.component-action:focus-visible{background:var(--primary);color:var(--primary-contrast)}

  .component-action--danger{border-color:#dc2626;color:#dc2626}

  .component-action--danger:hover,.component-action--danger:focus-visible{background:#dc2626;color:#fff}

  .component-summary{font-size:.9rem;color:var(--fg-muted,#4b5563)}

  .components-empty{font-style:italic;color:#6b7280}

  .component-tree{list-style:none;padding-left:1rem;margin:1.5rem 0;display:flex;flex-direction:column;gap:.35rem}

  .component-tree ul{list-style:none;padding-left:1.25rem;margin:.35rem 0 0 0;display:flex;flex-direction:column;gap:.35rem;border-left:1px dashed rgba(0,0,0,.2)}

  [data-theme="dark"] .component-tree ul{border-color:rgba(255,255,255,.25)}

  .component-item{position:relative}

  .component-node{border:1px solid rgba(0,0,0,.15);border-radius:.4rem;padding:.6rem .75rem;background:rgba(0,0,0,.02);display:flex;flex-direction:column;gap:.6rem}

  [data-theme="dark"] .component-node{border-color:rgba(255,255,255,.2);background:rgba(255,255,255,.05)}

  .component-node__header{display:flex;justify-content:space-between;align-items:flex-start;gap:.75rem;flex-wrap:wrap}

  .component-node__info{display:flex;flex-direction:column;gap:.25rem}

  .component-node__info strong{font-weight:600;font-size:1rem}

  .component-alias{font-size:.75rem;color:#4b5563}

  [data-theme="dark"] .component-alias{color:#cbd5e1}

  .component-meta{font-size:.75rem;color:#4b5563}

  [data-theme="dark"] .component-meta{color:#cbd5e1}

  .component-actions{display:flex;flex-wrap:wrap;gap:.35rem}

  .component-node__details{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:.5rem;margin:0}

  .component-node__details dt{font-weight:600;font-size:.75rem;color:#4b5563;margin-bottom:.2rem}

  [data-theme="dark"] .component-node__details dt{color:#cbd5e1}

  .component-node__details dd{margin:0;font-size:.85rem}
  ...
  .component-field--media{display:flex;flex-direction:column;gap:.5rem}

  .component-media-label{font-weight:600;font-size:.85rem}

  .component-media-toggle{display:flex;flex-wrap:wrap;gap:.75rem;font-size:.85rem}

  .component-media-toggle input{margin:0 .25rem 0 0}

  .component-media-panel{display:flex;flex-direction:column;gap:.35rem}

  .component-media-panel.hidden{display:none}

  .component-color-picker{display:flex;align-items:center;gap:.5rem}
  .component-color-chip{display:inline-block;width:1rem;height:1rem;border-radius:999px;border:1px solid rgba(0,0,0,.2);margin-right:.35rem;vertical-align:middle;background:var(--chip-color,#000)}
  [data-theme="dark"] .component-color-chip{border-color:rgba(255,255,255,.35)}

  .component-color-picker input[type="color"]{width:2.5rem;height:2.5rem;padding:0;border:1px solid var(--fg);border-radius:.35rem;background:transparent}
  ...
  .component-select .select{width:100%}
